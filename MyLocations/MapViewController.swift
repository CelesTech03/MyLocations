...
//  MyLocations
//
//  Created by Celeste Urena on 11/1/22.
//
...
class MapViewController: UIViewController {
    ...
    // Shows a region on the map
    func region(for annotations: [MKAnnotation]) -> MKCoordinateRegion {
        let region: MKCoordinateRegion
        switch annotations.count {
            /* If there are no annotation, center the map on the user's curr position
             If there is only one annotation, center the map on that one annotation.
             If there are two or more annotations, calculate the extent of their reach and add a little padding. */
        case 0:
...
// Creates custom annotation views
